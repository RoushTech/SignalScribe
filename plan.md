...
| **Voice- or mode-gated channel auto-creation** | Unknown frequencies become channels when a transmission either *sounds like voice* — a cheap capture-side gate on the demodulated audio (syllabic envelope modulation at ~2–8 Hz + speech-band energy ratio + minimum duration), confirmed later by the worker-side VAD before transcription — or carries a modulation the classifier can *name*, which is the same kind of evidence: it says what the frequency is. Everything else on an unknown frequency (birdies, spurs, carrier-only kerchunks, and digital we cannot yet identify) is discarded, not posted. Once a channel exists (auto or manual), everything its squelch opens is recorded — the gate governs channel *creation*, not capture on known channels. |
| **APRS decoded, not inferred** | The soft TNC (`SignalScribe.Modem`, vendored MIT from DireControl) runs on the discriminator and a CRC-valid AX.25 frame identifies the frequency outright — no statistic competes with that. Measured, the 16 kHz audio path only decodes within ~2 kHz of a filterbank bin and 144.390 sits 2.5 kHz off it, so the tap is pre-de-emphasis. Frames become `Segment` rows so they inherit FTS search, but never count as speech. It costs ~4% of a core per gate against the tone detector's 0.1%, so it is capped and spent only on unknown frequencies and known data channels. |
| **Digital framing needs no fine DDC** | The open per-channel fine-DDC item was assumed to block digital voice framing. Measured (`SymbolRecoveryProbe`), it does not: through the real channelizer and discriminator, 4800-baud symbols recover with 0% error out to ±3.75 kHz off-bin and 0.39% at ±5 kHz — the worst case the 12.5 kHz grid allows — which is well inside what DMR's and D-STAR's FEC absorbs. So framing is built on the discriminator, and the channelizer's prototype filter is left alone. Fine DDC remains worth doing for *analog audio quality* on off-grid channels (`OffGridAudioTests`), which is what it was originally for. |
| **Mode classification from deviation levels** | A digital signal dwells at a few discrete deviations and speech does not, so a histogram of the discriminator separates them for ~0.02% of a core per open gate. Two-level modes are named (D-STAR ±1.2 kHz vs POCSAG ±4.5 kHz are nearly 4× apart); four-level modes are reported only as "digital", because DMR and P25 sit 8% apart while the channel filter compresses levels 4–6% and transmitter error adds as much again. Sync-pattern correlation in the per-mode framers is what names those, and a CRC-valid AX.25 frame is what names APRS. The classifier triages; the decoders decide. |
